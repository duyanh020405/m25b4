...
export default function RotatingMessages() {
  ...
  useEffect(() => {
    const userOnl = localStorage.getItem('userOnl');
    if (!userOnl) {
      router.push('/login');
    }
  }, [router]);
          ...
          <div key={product.id} className="p-4 border rounded">
            ...
            <div className="flex flex-col gap-5">
              <div className="flex gap-15">
                <Button className="mt-2" onClick={() => viewDetails(product.id)}>Mua ngay</Button>
                <Button className="mt-2" onClick={() => viewDetails(product.id)}>Xem chi tiết</Button>
              </div>
              <a href="#" onClick={() => viewDetails(product.id)}>Bình luận về sản phẩm, ấn tại đây</a>
            </div>
          </div>
        ))}
      </div>
